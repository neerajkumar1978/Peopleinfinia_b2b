	...
	admin_cv_shortlist: function (req, res) {
		...
		Usercandidate.findOneAndUpdate({ _id: mongoose.Types.ObjectId(req.body._id), 'candidates._id': mongoose.Types.ObjectId(req.body.candidate_id) }, { $set: { 'candidates.$.ey_shortlist': true } }, function (err, result) {
			if (err) {
				...
				// var htmlContent = spark.CandidateShortlistedforAdmin(
				// 	req.body.job_title,
				...
				// 	  console.log('Candidate Shortlisted for Admin.');
				// 	  res.send({
				// 		responseMessage: "Candidate Shortlisted for Admin mail Send suceesfully",
				// 	//	responseCode: 200
				// 	})
				// 	})
				// 	.catch(err => {
				// 		res.send({
				// 			responseMessage: "confirm_slot: Unable to send mail",
				//   //	responseCode: 500
				// 		})
